    ...
    <div>
      ...
      {error ? (
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr>
              <th style={{ border: '1px solid #000', padding: '8px' , color:'#000'}}>Username</th>
              <th style={{ border: '1px solid #000', padding: '8px' , color:'#000'}}>Email</th>
              <th style={{ border: '1px solid #000', padding: '8px' , color:'#000'}}>Actions</th>
            </tr>
          </thead>
          <tbody>
            {users.map((user, index) => (
              <tr key={index} style={{ border: '1px solid #000' }}>
                <td style={{ padding: '8px', color:'#000' }}>{user.username}</td>
                <td style={{ padding: '8px', color:'#000' }}>{user.email}</td>
              ...
              {userSongs.length > 0 ? (
                <ul>
                  {userSongs.map((song, index) => (
                    <li style={{ color: 'black'  }} key={index}>{song.title}</li>
                  ))}
                </ul>
              ) : (
